...
class TestResolveAppRoot:
    ...
    def test_returns_cli_value_when_provided(self, tmp_path: Path) -> None:
        ...
        app_dir.mkdir()
        (app_dir / "trellis.py").write_text("config = None")
    ...
    def test_cli_takes_precedence_over_env(
        ...
        cli_dir.mkdir()
        (cli_dir / "trellis.py").write_text("config = None")

        env_dir = tmp_path / "env_app"
        env_dir.mkdir()
        (env_dir / "trellis.py").write_text("config = None")
    ...
    def test_file_path_uses_parent_directory(self, tmp_path: Path) -> None:
        """When CLI points to trellis.py file, parent directory is used."""
        app_dir = tmp_path / "myapp"
        app_dir.mkdir()
        trellis_file = app_dir / "trellis.py"
    ...
    def test_raises_when_no_trellis_py(self, tmp_path: Path) -> None:
        """Raises FileNotFoundError when directory has no trellis.py."""
        empty_dir = tmp_path / "empty"
        empty_dir.mkdir()

        with pytest.raises(FileNotFoundError, match=r"trellis\.py not found"):
            resolve_app_root(cli_value=empty_dir)

    def test_raises_when_auto_detect_fails(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """Raises FileNotFoundError when auto-detect finds no trellis.py."""
        empty_dir = tmp_path / "empty"
        empty_dir.mkdir()
    ...
    def test_env_path_file_uses_parent_directory(
        ...
        """When ENV points to trellis.py file, parent directory is used."""
        app_dir = tmp_path / "myapp"
        app_dir.mkdir()
        trellis_file = app_dir / "trellis.py"
        trellis_file.write_text("config = None")
