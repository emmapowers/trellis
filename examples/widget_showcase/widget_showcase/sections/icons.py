...
def Icons() -> None:
    ...
    with w.Column(gap=16):
        w.Label(text="Common Icons", font_size=12, color=theme.text_secondary, bold=True)
        with w.Row(gap=16):
        ...
        w.Label(text="Status Icons", font_size=12, color=theme.text_secondary, bold=True)
        with w.Row(gap=16):
            w.Icon(name=IconName.INFO, size=20, color=theme.info)
            w.Icon(name=IconName.CHECK_CIRCLE, size=20, color=theme.success)
            w.Icon(name=IconName.ALERT_TRIANGLE, size=20, color=theme.warning)
            w.Icon(name=IconName.ALERT_CIRCLE, size=20, color=theme.error)

        w.Label(text="Chart Icons", font_size=12, color=theme.text_secondary, bold=True)
        with w.Row(gap=16):
            w.Icon(name=IconName.BAR_CHART, size=20)
            w.Icon(name=IconName.LINE_CHART, size=20)
            w.Icon(name=IconName.PIE_CHART, size=20)
            w.Icon(name=IconName.TRENDING_UP, size=20, color=theme.success)
            w.Icon(name=IconName.TRENDING_DOWN, size=20, color=theme.error)

        w.Label(text="Sizes", font_size=12, color=theme.text_secondary, bold=True)
