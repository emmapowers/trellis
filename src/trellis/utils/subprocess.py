...
def start_child_process(cmd: list[str], **popen_kwargs: tp.Any) -> subprocess.Popen[tp.Any]:
...
def stop_child_process(process: subprocess.Popen[tp.Any], timeout: float = 5.0) -> None:
...
def _stop_posix(process: subprocess.Popen[tp.Any], timeout: float) -> None:
...
def _start_windows(cmd: list[str], **popen_kwargs: tp.Any) -> subprocess.Popen[tp.Any]:
    """Start a child process on Windows with Job Object cleanup."""
    import ctypes  # noqa: PLC0415 — only needed on Windows
    import ctypes.wintypes  # noqa: PLC0415  # pyright: ignore[reportAttributeAccessIssue]
...
def _stop_windows(process: subprocess.Popen[tp.Any], timeout: float) -> None:
