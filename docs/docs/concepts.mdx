...
def Counter() -> None:
    count = state_var(0)
    h.P(f"Count: {count.value}")
```

The state persists across re-renders. Each time the component renders, it gets the same slot-local state object.
...
1. When the component renders, it reads `count.value`
...
def Form() -> None:
    ...
    if submitted.value:
        ...
        Button(text="Submit", on_click=lambda: submitted.set(True))
```
