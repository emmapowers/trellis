...
Use either `h.Style(...)` for typed CSS or a raw `style` dictionary with DOM-style CSS names:
...
Raw style dictionaries use DOM-style names:

- `background-color`
- `font-size`
- `border-radius`

Structured `h.Style(...)` uses snake_case field names:
...
@component
def SettingsPanel():
    with h.Div(style={"padding": "20px", "font-family": "system-ui"}):
        h.H1("Settings", style={"margin-bottom": "20px"})
