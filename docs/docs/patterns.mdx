...
def App():
    ...
    with w.Column(gap=12):
        h.H2("Product")
        h.P("A great product for all your needs.")
        ...
        if state.show_details:
            with h.Div(style={"marginTop": "16px", "padding": "12px", "backgroundColor": "#f5f5f5"}):
                h.P("Price: $99.99")
                h.P("In stock: Yes")
                h.P("Ships in 2-3 days")
...
def App():
    ...
    with w.Column(gap=12):
        ...
        if not state.items:
            h.P("No items yet.", style={"color": "#888"})
        else:
            for item in state.items:
                with w.Row(gap=8, align="center"):
                    h.Span(item, style={"flex": "1"})
...
def Stat(label: str, value: str):
    with w.Row(gap=8):
        h.Span(f"{label}:", style={"fontWeight": "bold"})
        h.Span(value)
