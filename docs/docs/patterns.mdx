...
def App() -> None:
    ...
    with w.Column():
        Header(name=user_name.value)  # Pass state as prop
...
def App():
    ...
    with w.Column(gap=12):
        w.Heading(text=f"Count: {count.value}", level=2)
        Controls(
            on_increment=lambda: count.set(count.value + 1),
            on_decrement=lambda: count.set(count.value - 1),
            on_reset=lambda: count.set(0),
        )
...
def App():
    ...
    with w.Column(gap=12):
        ...
        w.Button(
            text="Hide Details" if show_details.value else "Show Details",
            on_click=lambda: show_details.set(not show_details.value),
        )
