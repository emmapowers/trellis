...
def MyApp():
    ...
    with w.Column(gap=16):
        ...
        w.Label(text=f"Mode: {state.mode.value}")
...
# Properties
state.mode           # ThemeMode.SYSTEM | LIGHT | DARK
state.resolved_theme # Actual theme: LIGHT or DARK
...
| **Accent** | `accent_primary`, `accent_primary_hover`, `accent_primary_active`, `accent_subtle` |
| **Success** | `success`, `success_bg`, `success_border` |
| **Error** | `error`, `error_bg`, `error_border`, `error_hover` |
| **Warning** | `warning`, `warning_bg`, `warning_border` |
| **Info** | `info`, `info_bg`, `info_border` |
| **Shadows** | `shadow_sm`, `shadow_md`, `shadow_lg` |
| **Focus** | `focus_ring_color` |

Each token is a CSS variable reference (e.g., `var(--trellis-bg-page)`) that automatically updates when the theme changes.
