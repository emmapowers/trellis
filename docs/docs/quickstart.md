...
3. **`with Column():`** — Components nest using Python's `with` blocks. `Column` arranges children vertically.

4. **`h.H1()`, `h.P()`** — HTML elements. The `h` module provides all standard HTML tags.

5. **`Button(..., on_click=...)`** — A widget with an event handler. When clicked, it calls `state.increment()`.
